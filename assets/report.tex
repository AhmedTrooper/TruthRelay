...
\begin{figure}[t]
  ...
  \begin{tikzpicture}[
      ...
      phone/.style={rectangle,rounded corners=3pt,draw=trSky,fill=trSky!10,
                    minimum width=32mm,minimum height=12mm,align=center, thick},
      laptop/.style={rectangle,rounded corners=3pt,draw=trEmerald,
                    fill=trEmerald!15,minimum width=38mm,minimum height=14mm,
                    align=center, thick},
      admin/.style={rectangle,rounded corners=3pt,draw=trViolet,
                    fill=trViolet!15,minimum width=32mm,minimum height=12mm,
                    align=center, thick},
      hotspot/.style={ellipse,draw=trSlate,fill=trSlate!10,
                    minimum width=26mm,minimum height=8mm,align=center, thick},
    ...
    % Nodes
    \node[laptop] (relay) {\textbf{Edge Server (Laptop)}\\ \scriptsize Axum + SQLite (0.0.0.0:8080)\\ \scriptsize Verifies Ed25519};
    \node[hotspot,above=12mm of relay] (ap) {\textbf{Local Wi-Fi Hotspot}\\ \scriptsize (No Internet)};
    \node[admin,left=15mm of ap] (web) {\textbf{Vue 3 PWA}\\ \scriptsize Moderator Dashboard};
    
    \node[phone,below left=18mm and -10mm of relay] (phoneA) {\textbf{Phone A}\\ \scriptsize Connected to Hotspot};
    \node[phone,below right=18mm and 5mm of relay] (phoneB) {\textbf{Phone B}\\ \scriptsize Fully Offline};
    \node[phone,below=20mm of phoneB] (phoneC) {\textbf{Phone C}\\ \scriptsize Fully Offline};
    \node[phone,left=20mm of phoneC] (phoneD) {\textbf{Phone D}\\ \scriptsize Fully Offline};
    ...
    % Mesh gossip
    \draw[mesh] (phoneA) -- node[above,font=\tiny,trAmber]{Wi-Fi Direct} (phoneB);
    \draw[mesh] (phoneB) -- node[right,font=\tiny,trAmber]{BLE} (phoneC);
    \draw[mesh] (phoneC) -- node[above,font=\tiny,trAmber]{Wi-Fi Direct} (phoneD);
    \draw[mesh] (phoneA) -- node[left,font=\tiny,trAmber]{BLE} (phoneD);
  ...
  \end{tikzpicture}
  \caption{100\% Offline Edge-Node Architecture. The backend runs on a laptop serving a local Wi-Fi hotspot. Phone A pulls data and virally seeds it to the offline mesh (Phones B, C, D) via BLE and Wi-Fi Direct, completely independent of the global internet.}
  \label{fig:overview}
\end{figure}
...
Crucially, the system is designed to operate completely independent of the global internet. During a crisis, a local laptop hosts the Axum relay server over a self-hosted Wi-Fi hotspot (exposed on \texttt{0.0.0.0:8080}). Mobile devices configure their environment variables (via \texttt{--dart-define}) to point to this specific local IP address and port. The Docker setup requires no heavy external databases (e.g., PostgreSQL or Redis) because it leverages SQLite directly, needing only the base OS environment to compile the code. Once the first phone connects to this local hotspot and fetches the data, it leaves the area and virally propagates the information mobile-to-mobile across the offline mesh via BLE and Wi-Fi Direct.
...
\begin{itemize}[leftmargin=*]
  ...
  \item \textbf{Connectivity}: a 3-second-debounced
        ...
        screen.
  \item \textbf{Logging}: \texttt{AppLogger} (\texttt{package:logger}) delivers formatted, emoji-enhanced, leveled console output across startup, outbox sync, and P2P mesh sessions.
\end{itemize}
...
\texttt{GET  /healthz} & Liveness probe.\\
\bottomrule
\end{tabularx}
\end{center}
...
Five routes (\emph{Mission Control}, \emph{Bulletins}, \emph{Requests},
\emph{Moderators}, and \emph{Sync}). Features 1-Click Moderator Keygen \& Server Registration directly inside the sign bulletin modal, auto-negotiates admin tokens (\texttt{dev-token-change-me}, \texttt{change-me}), and displays live relay health probes with 1-click retry banners. Everything is signature-aware: the \emph{Signed Bulletin Inspector} renders the exact canonical-bytes, the \texttt{sha256}, and the base64 signature the server will verify.
...
BLE Battery Drain & ~18--22\% battery over 12\,h & Screen-off duty throttling (5\,s scan / 60\,s sleep; $<$5\% drain).\\
Unsigned Requests & Help requests tagged UNVERIFIED & Per-user Ed25519 client keypairs generated in mobile storage.\\
